...
class Http_req_Handler:
    ...
    #Perform GET of requested stock   
    # function to process the GET request

    def do_GET(self,stock_name):
        connect = HTTPConnection('catalog_service', 8001) #Connect to the catalog server 
    ...
    def do_POST(self,content_type,body):
        ...
        #print("dic is",content)
        body=json.dumps(body)
        connect2 = HTTPConnection('orders_service', 8080) #connect to order service
        print("connected to orders service")
        req_endpoint='/trade_stocks'
        connect2.request('POST',req_endpoint,body,content) #Connect to the orders server 
        print("sent to orders service")
        response = connect2.getresponse() 
        response_data = response.read()
        data=pickle.loads(response_data)
        print("response from orders service is",data)
...
PORT=12345
IP='frontend_service'                                   #give the IP as the containerservice name
s=socket.socket()
s.bind((IP,PORT))
s.listen(5)
